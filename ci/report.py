...
from ci.entities import ContractorData
import duckdb
...
def generate_report(year: int, month: int) -> None:
    ...
    for item in contractor_data:
        working_days = _get_working_days_in_month(year, month, item.start_date)
...
def _get_working_days_in_month(year: int, month: int, start_date: date) -> float:
    days_in_month = monthrange(year, month)[1]
    days = [date(year, month, day) for day in range(1, days_in_month + 1)]
    week_days_in_month = sum(
        [1 if d.weekday() in WEEK_DAYS and start_date <= d else 0 for d in days]
    )
